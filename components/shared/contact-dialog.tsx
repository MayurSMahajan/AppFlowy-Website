...
interface ContactDialogProps {
  ...
  title?: string;
  /** Fallback `source` reported to the API when the URL has no `?source=`. */
  source?: string;
  /**
   * Preselects the deployment mode. The pricing page passes its current
   * Cloud/Self-hosted tab; elsewhere there is no such context, so the user
   * picks it in the form.
   */
...
export function ContactDialog({
  ...
  source: defaultSource = 'pricing-page',
  ...
  // Keep the form in sync when the dialog is reopened with a different mode
  useEffect(() => {
    setFormData(prev => ({
      ...prev,
      deploymentMode: getDisplayDeploymentMode(deploymentMode),
  ...
  useEffect(() => {
    const sourceParam = searchParams.get('source') || defaultSource;
  ...
  const handleClose = () => {
    // The `?action=contact` / `?source=` deep-link mechanism only exists on
    // the pricing page (see qa-section.tsx and questions-section.tsx); on
    // any other page these params belong to something else and must be left
    // alone.
    const hasAction = pathname === '/pricing' && searchParams.has('action');
    const hasSource = pathname === '/pricing' && searchParams.has('source');
